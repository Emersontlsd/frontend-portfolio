...
const Feedback = () => {
  ...
  //Estados dos campos formulários
  const [ nome, setNome] = useState('');
  const [ telefone, setTelefone ] = useState('');
  const [ mensagem, setMensagem ] = useState('');
  const [rating, setRating] = useState(5);
  
  // estado para armazenar feedbacks que vem do banco
  const [ feedbacks, setFeedbacks ] = useState([]);
  
  // buscar feedback no banco de dados
  const carregarFeedbacks = async () => {
    try {
      const resposta = await fetch(`${import.meta.env.VITE_API_URL}/api/feedbacks`);
  ...
  // carregar feedbacks assim que a tela abrir
  useEffect(() => {
    carregarFeedbacks();
  }, []);
    ...
    <div className='min-h-screen pt-32 px-6 pb-20 max-w-5xl mx-auto'>
      ...
      <div className='grid md:grid-cols-3 gap-8'>
        ...
        <form onSubmit={handleSubmit} className='md:col-span-1 bg-slate-900/50 p-6 rounded-2xl border border-white/10 h-fit backdrop-blur-md'>
          <h3 className="text-white font-bold mb-4 uppercase text-sm tracking-widest">Deixe sua avaliação</h3>
          ...
          <div className='space-y-4'>
            ...
            <div className='flex flex-col items-center gap-2'>
              <span className='text-xs text-slate-400 uppercase'>Sua nota</span>
              <div className='flex gap-2'>
                {[1, 2, 3, 4, 5].map(s => (
                  <Star
                    ...
                    size={22}
                    onClick={() => setRating(s)}
                    className={`${s <= rating ? "fill-neon-cyan text-neon-cyan" : "text-slate-600"} cursor-pointer hover:scale-110 transition-transform`}
                  />
                ))}
              </div>
            </div>

                <button type='submit' className="w-full bg-neon-pink hover:brightness-110 text-white py-3 rounded-lg font-black text-sm tracking-widest shadow-[0_0_15px_rgba(255,0,229,0.2)] transition-all">
                  PUBLICAR
                </button>
          </div>
        </form>
        ...
        <div className='md:col-span-2 space-y-6'>
          {feedbacks.length === 0 ? (
            <p className='text-slate-500 text-center italic'>Nenhum feedback ainda. Seja o primeiro</p>
              ...
              <div key={f.id} className="bg-slate-800/20 p-6 rounded-2xl border border-white/5 border-l-4 border-l-neon-cyan hover:bg-slate-800/30 transition-all">
                <div className='flex justify-between items-center mb-4'>
                  ...
                  <div className='flex gap-1'>
                    {Array.from({ length: f.estrelas}).map((_, i) => (
                      <Star key={i} size={14} className='fill-white text-white' />
                    ))}
                  </div>
                </div>
                <p className='text-slate-300 leading-relaxed italic'>"{f.mensagem}"</p>
              </div>
            ))    
          )}
        </div>
